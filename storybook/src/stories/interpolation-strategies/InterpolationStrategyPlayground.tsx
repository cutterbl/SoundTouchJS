...
import type { RateTransposerInterpolationStrategy } from '@soundtouchjs/core';
import {
  type ChangeEvent,
  type JSX,
  useCallback,
...
interface InterpolationStrategyPlaygroundProps {
  readonly title: string;
  readonly description: string;
  readonly explanation: string;
  readonly codeSample: string;
  readonly strategyId: RateTransposerInterpolationStrategy;
  readonly initialParams: Record<string, number>;
  readonly paramControls: readonly ParamControl[];
}
...
function RangeControl({
  ...
  return (
    <label style={{ display: 'block' }}>
      {label}: {(display ?? displayNumber)(value)}
      <input
        type="range"
        min={min}
        max={max}
...
export function InterpolationStrategyPlayground({
  title,
  description,
  explanation,
  codeSample,
  ...
  initialParams,
  paramControls,
}: InterpolationStrategyPlaygroundProps): JSX.Element {
  ...
  }, [pitch, pitchSemitones, rate, volume]);
  ...
  const createGraph = useCallback(async (): Promise<void> => {
    ...
    const soundTouchNode = new SoundTouchNode({
      context,
      interpolationStrategy: { id: strategyId, params },
    });
  ...
  const isStrategyParamsDisabled = isLoading || isPlaying;
  ...
  useEffect(() => {
    ...
    soundTouchNodeRef.current.setInterpolationStrategy({
      id: strategyId,
      params,
    });
    soundTouchNodeRef.current.setInterpolationStrategyParams(params);
  }, [params, strategyId]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <legend>{title} tunable params</legend>
          <div
            style={{
              display: 'grid',
              gap: '0.75rem',
          ...
          >
            {paramControls.map((control) => (
              <RangeControl
                ...
                ticks={control.ticks}
                listId={`${strategyId}-${control.key}-ticks`}
                disabled={isStrategyParamsDisabled}
                display={control.display}
              />
            ))}
          </div>
          <p style={{ marginTop: '0.5rem', marginBottom: 0, color: '#6b7280' }}>
            Strategy params are disabled while playback is active.
          </p>
        </fieldset>
      </div>
...
export type { DatalistTick, ParamControl };