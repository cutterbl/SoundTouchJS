...
describe('AbstractSamplePipe', () => {
  describe('constructor with createBuffers=true', () => {
    it('creates input and output buffers', () => {
      const pipe = new AbstractSamplePipe({
        createBuffers: true,
        inputBufferFactory: () => new FifoSampleBuffer(),
        outputBufferFactory: () => new FifoSampleBuffer(),
      });
      expect(pipe.inputBuffer).toBeInstanceOf(FifoSampleBuffer);
      expect(pipe.outputBuffer).toBeInstanceOf(FifoSampleBuffer);
    });

    it('throws when factories are missing', () => {
      expect(() => new AbstractSamplePipe({ createBuffers: true })).toThrow(
  ...
  describe('constructor with createBuffers=false', () => {
    it('sets buffers to null', () => {
      const pipe = new AbstractSamplePipe({ createBuffers: false });
